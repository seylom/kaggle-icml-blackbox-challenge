...
def get_layer_trainer_sgd_autoencoder(layer, trainset,savepkl):
    ...
    train_algo = SGD(
              learning_rate = 0.1,
              cost = MeanSquaredReconstructionError(), #MeanBinaryCrossEntropy() 
              batch_size =  100,
...
def main():
    ...
    layers.append(get_denoising_autoencoder(structure[0],0.1))
    # layer 1: denoising AE
    layers.append(get_denoising_autoencoder(structure[1],0.2))
     # layer 1: denoising AE
    layers.append(get_denoising_autoencoder(structure[2],0.3))
